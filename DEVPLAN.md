...
## Phase 3 to 10 — Stateless FastAPI Backend (Supabase)

> **ARCHITECTURE UPDATE**: We pivoted away from using SQLAlchemy, Asyncpg, and Alembic migrations. Instead, the entire backend is built using the official `supabase` Python client. All database interactions are completely stateless REST API calls.

### Implemented Architecture
- **No `models/` directory**: Since we aren't using an ORM, we rely purely on Pydantic `schemas/` to define inputs and outputs.
- **`core/database.py`**: Initializes the global `supabase.Client` using the `.env` configuration.
- **`core/dependencies.py`**: Contains `get_current_user` and `require_role`, which decode JWTs and query the `users` table via Supabase to enforce role-based access control.
- **Vercel Serverless Ready**: Because there are no persistent database connections pooling in the background, this backend is 100% optimized for cold starts on Vercel's serverless edge.

### Completed API Routers (`backend/routers/`)
1. **`auth.py`**: Custom JWT authentication (`/login`, `/register`, `/refresh`, `/me`).
2. **`menu.py`**: Menu items and Categories CRUD.
3. **`cart.py`**: Stateless cart management using Supabase nested queries (`cart_items(*, menu_items(*))`).
4. **`orders.py`**: Checkout logic, status updates, scheduling, and cancellation. Triggers automatic notifications via a helper utility when order statuses change.
5. **`payments.py`**: Razorpay checkout integration with secure HMAC SHA256 signature verification on the `/verify` endpoint.
6. **`favourites.py`**: Students bookmarking favorite dishes.
7. **`ratings.py`**: Students reviewing completed (`picked_up`) orders. Updates the `rating_avg` on the menu item.
8. **`notifications.py`**: Fetch and read-status management for user notifications.
9. **`users.py`**: Admin controls to suspend/delete users.
10. **`settings.py`**: Admin controls for global canteen flags (e.g., `is_accepting_orders`).

### Entrypoint (`backend/api/index.py`)
All routers are successfully mounted into the main FastAPI application with their respective prefixes.
